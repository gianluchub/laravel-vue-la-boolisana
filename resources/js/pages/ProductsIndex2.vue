            ...
            axios.get('/api/shapes').then(response => {
                ...
                self.shapes.forEach(shape => {
                    axios.get('/api/products/shapes/' + shape.id).then(response => {
                        // creo una nuova chiave nell'oggetto 'products' con la label corrispondente (reactivity) => https://vuejs.org/v2/api/#Vue-set
                        Vue.set(self.products, shape.id, response.data.results);

                        // Alternativa al Vue.set per rendere reattiva l'aggiunta dei prodotti per ogni forma
                        // self.products[shape.id] = response.data.results;
                        // self.$forceUpdate();
                    });
                });
            });
